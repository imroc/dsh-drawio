...
- **画板 tab 的可见性由侧栏布局决定**：宽视口下它就是侧栏里的一个 tab；窄到放不下并排（< 768px）时，侧栏会切成**全屏面板**，画板铺满屏幕、用工具栏的「关闭画板」退出（此时画面由侧栏的画板 tab 提供，插件只补了一条窄屏定位修正与一个抽屉入口）。**注意**：插件会往 profile 注入一条仅作用于 `max-width: 767px` 的样式，只针对侧栏自己的稳定属性（`[data-rightbar-col]`、`[data-sidebar-right-panel]`），用于修正移动 shell 下侧栏全屏面板的包含块塌陷；桌面端不受影响。
