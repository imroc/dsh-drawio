...
- **打开方式**（两种）：
  - 会话标题栏右上角的 **画板按钮**（挨着官方的侧栏展开按钮）——点一下就把侧栏连同画板一起打开；
...
| `src/client/header-entry.ts` | 会话标题栏右上角的画板按钮（DOM 注入，紧贴官方侧栏展开按钮） |
...
- **画板 tab 的可见性由侧栏布局决定**：DSH 的布局算法在窄视口（< 1024px）下会把右侧栏收起（可用宽度不足 300px 时右侧栏宽度算作 0）。所以手机端的建议用法是：先在桌面端把画板 tab 开出来（侧栏的 tab 布局是按会话持久化的），再在手机上查看。
