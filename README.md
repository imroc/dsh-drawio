...
- **窄屏（≤767px）改为全屏覆盖层**：手机端 frame 被移动壳压成单列，并排放不下——此时画板铺满视口（落在壳体固定顶栏之下），对话保持全屏在下方；退出用工具栏的**「关闭画板」**（该按钮在任何宽度都可达：手机上侧边栏是抽屉，入口行点不到）
- **AI 跟进节奏**：agent 改图时**实时**活动会在宽屏自动展开画板；**重放的历史活动只更新内容、不弹面板**（否则每次刷新都会把刚关掉的画板弹回来），**窄屏也不自动展开**以免打断输入。无论是否展开，画板都在静默跟随 agent 最新改的文件，打开即最新
...
- **独立页面 + 弹回**：「独立页面」在新标签页打开全屏官方编辑器；独立页的「保存」写回工作区、「弹回画板」保存并在侧栏画板显示后自动关闭标签页
